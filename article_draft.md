...
У CUPED мягкая репутация: кажется «безопасной поправкой». На практике так бывает не всегда.
...
Другая постановка: 88% относительно стабильных пользователей и 12% «шумных» (power users). В коде у последних стандартное отклонение внутриюнитного шума в пять раз выше, чем у остальных (`power_sd=5`, `stable_sd=1` в `simulate_power_user_dataset`), плюс общий разброс уровня baseline.
...
В **этом сценарии** комбинация CUPED и VWE даёт наименьшую разбросность и RMSE: методы бьют по разным компонентам шума. Обобщать на любой прод без проверки данных нельзя.
...
Не «включать всё сразу», а смотреть на структуру шума.
...
Такой уровень оговорок в техническом тексте обычно полезнее громкого заголовка без условий.
...
Вопрос при выборе инструмента звучит не «что моднее», а **какая структура шума у метрики в конкретном эксперименте** — и подтверждается ли это данными, а не только постановкой симуляции.
