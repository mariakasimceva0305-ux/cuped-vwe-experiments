...
Минимальный воспроизводимый **research companion**: две симуляции на синтетике, где отдельно смотрят на чувствительность **CUPED** и на **VWE** (variance-weighted estimation). Это не библиотека под прод и не пакет для пайплайна — только прозрачный код и заранее сохранённые результаты.

## What this repo is

- две явные постановки (корреляция pre/post и «шумные» доля пользователей);
